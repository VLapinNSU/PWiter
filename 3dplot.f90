...
real, dimension(Nmax,Nmax) :: F ! если не используете описание переменных в комментариях, то можно писать через запятую 
real, dimension(Nmax,Nmax) :: G  
real, dimension(Nmax) :: P, W     ! как здесь
real, dimension(2,2) :: T, relax  
real, dimension(2) :: P0, W0, P1, W1, Wn
real :: eps, qin, pout    ! но лучше комментарии какие-то делать
integer :: i, mu, E, h, dt   ! надо все переменные описывать (мы же запретили implicit none)
